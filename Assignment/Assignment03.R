...
library(RColorBrewer)
pal <- brewer.pal(5, "OrRd")
spplot(admin_utm, "popden", col.regions = pal, at = brk$brks, main = "서울시 구별 인구밀도")
...
gmap.map <- ggmap(gmap) + geom_point(data = mapdata, aes(x = 경도, y = 위도, colour = 행정동코드_num, size = 최대수용인원_num), alpha = 1)
gmap.map
